...
def htree_compute_scores_and_select_kernel(
    ...
    if not is_bottom_layer:
        ...
        padding_value = -1e10  # finite padding to avoid NaNs inside argsort
        ...
        # 4.3 不合法的节点已经在存储时设为 -inf，无需额外处理
        
        # 4.4 对 8192 个节点降序排序：基于 buffer_scores
        # 创建 original indices 数组 [0, 1, 2, ..., 8191] 追踪buffer位置
        n_dims: tl.constexpr = 13  # log2(8192) = 13
        n_outer: tl.constexpr = 1
        topk_sort_positions = tl.arange(0, MAX_CANDIDATES).to(tl.int32)
        
        # 降序排序：topk_sort_positions[i] 是排序后第i个元素在buffer中的原始位置
        buffer_scores, topk_sort_positions = argsort_v2(
            buffer_scores, topk_sort_positions, n_dims, n_outer, descending=True
        )
        
        # 4.5 提取前 TOP_K 个位置（使用 reshape 技巧）
        ...
        # 4.6 根据 all_node_indices 和 topk_buffer_positions 提取选出的全局节点索引 selected_node_indices_sorted（给下一层）
        selected_node_indices = tl.zeros([TOP_K], dtype=tl.int32)

        # 遍历 TOP_K 个位置，从 buffer_node_indices 中提取对应的全局节点索引
        for i_topk in tl.static_range(TOP_K):
            # 提取 topk_buffer_positions[i_topk]
            pos_mask = (tl.arange(0, TOP_K) == i_topk)
            target_pos = tl.sum(tl.where(pos_mask, topk_buffer_positions, 0))

            # 从 buffer_node_indices 中提取对应位置的值
            buffer_match = (o_buf == target_pos)
            selected_idx = tl.sum(tl.where(buffer_match, buffer_node_indices, 0))

            # 填充到 selected_node_indices[i_topk]
        ...
        # 对 selected_node_indices 进行升序排序（传给下一层）
        # 将 -1 替换为 MAX_IDX
        ...
        # selected_indices: 全局节点索引
        #tl.store(selected_indices + topk_offset + o_topk, selected_node_indices.to(tl.int32))
...
def htree_accumulate_non_topk_kernel(
    ...
    # topk_pos_vals: [TOP_K]，内容为 buffer 中的位置（可能包含 -1 padding）
    # 使用循环避免创建过大的 match matrix [8192, 512]
    topk_mask = tl.zeros([MAX_CANDIDATES], dtype=tl.int1)
    
    for i_topk in range(TOP_K):
        # 提取 topk_pos_vals[i_topk]
        pos_mask = (tl.arange(0, TOP_K) == i_topk)
        target_pos = tl.sum(tl.where(pos_mask, topk_pos_vals, 0))
        
        # 标记 buffer 中对应位置（无效位置会匹配到 -1，不会影响结果）
        buffer_match = (o_buf == target_pos) & (target_pos >= 0)
        topk_mask = topk_mask | buffer_match
